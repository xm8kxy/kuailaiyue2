...
class ArticleController extends ComController
{
    ...
    public function add()
    {
        ...
        $category = M('category')->field('id,pid,name')->where('type!=1')->order('o asc')->select();
    ...
    public function update($aid = 0)
    {
        ...
        $data['tp_sp'] = I('post.tp_sp', '', 'strip_tags');
        $data['city'] = isset($_POST['city']) ? $_POST['city'] : '';//城市
        $data['type'] = isset($_POST['type']) ? $_POST['type'] : '';//营地类别
        
        $data['jiage'] = isset($_POST['jiage']) ? trim($_POST['jiage']) : '';//价格
        $data['danwei'] = isset($_POST['danwei']) ? trim($_POST['danwei']) : '';//单位
        $data['leixing'] = isset($_POST['leixing']) ? trim($_POST['leixing']) : '';//类型
        $data['pingpai'] = isset($_POST['pingpai']) ? trim($_POST['pingpai']) : '';//品牌
        $data['address'] = isset($_POST['address']) ? trim($_POST['address']) : '';//地址
