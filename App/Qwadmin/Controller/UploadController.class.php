...
//                $spdz= '/public/qwadmin/img/loading.gif';

            $Img = $this->savesp($_FILES);
        }
        $BackCall = I('BackCall');
        $Width = I('Width');
        $Height = I('Height');
...
//
//if($Img){
//    $spdz= '/public/qwadmin/img/sczg.jpg';
//}
        $this->assign('Img', $Img);
        $this->assign('Height', $Height);
        $this->display('Upps');
    ...
    //qxs
    public function Uptp_sp()
    {
        $Img = I('Img');
        $Path = null;
        $type_file=explode('/', $_FILES['img']['type']);//获取文件后缀名
